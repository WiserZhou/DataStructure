#include "MGraph.h"
/**
 * *D[]表示当前所找到的原点到每个终点i的最短路径的长度，最短路径的初值都是源点到各点的路径长
 * *度，后来进行赋值的时候就是当前迭代点与各点的距离与之前已经填上的最短路径之间的比较
 * *final[]表示顶点i的最短路径是否求出，1则求出，0则没有求出，求出的就没必要再进行比较了
 * *P[]表示从源点到点i的当前最短路径上离i最近的那个点的编号，如果是-1，那么表示没有最
 * *短路，通过它可以标记最短路径的源头
 * v0表示源点
 */
void ShortestPath_DIJ(MGraph G, int v0, int *P, int *D)
{
    ...
    for (int i = 1; i < G.vexNum; ++i) // 剩下的G.vexNum-1个结点
    {
        ...
        for (int w = 1; w <= G.vexNum; ++w)
            if (!final[w] && (D[v] + G.arc[v][w].adj < D[w])) // 如果没有求出最短路的而且此时最短路径加上v-w之间的距离比之前的标记更短
            {
                D[w] = D[v] + G.arc[v][w].adj;
                P[w] = v; // 重新标记
            }
    } // for
