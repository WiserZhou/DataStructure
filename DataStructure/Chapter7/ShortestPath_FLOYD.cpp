...
// Floyd算法的空间复杂性主要体现在利用了一个二维数组，空间复杂性为O(n^2)
// 讨论：权值是否可以是负值？
...
// *P矩阵（路径矩阵）： 存储了图中任意两个顶点之间的最短路径上的中间顶点信息。P[i][j][k]表示从顶点i到顶点j的最短
// *路径上是否经过了顶点k。如果P[i][j][k]为真（TRUE），则表示最短路径经过了顶点k，否则表示没有经过。
// 在算法执行过程中，P矩阵也会不断更新。
...
void ShortestPath_FLOYD(MGraph G, PathMatrix P, DistanceMatrix &D)
{
    ...
    for (int v = 1; v <= G.vexNum; ++v)
        for (int w = 1; w <= G.vexNum; ++w)
        {
            cout << "Shortest distance from " << v << " to " << w << ": " << D[v][w] << endl;
            cout << "Path: ";
            for (int i = 1; i <= G.vexNum; ++i)
                if (P[v][w][i])
                    cout << i << " ";
